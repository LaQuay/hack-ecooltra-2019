...
export default (class HeatmapLayer extends MapLayer {
  ...
  redraw(): void {
    ...
    const maxZoom =
      this.props.maxZoom === undefined ? this.context.map.getMaxZoom() : this.getMaxZoom(this.props)

    const v = 1 / Math.pow(2, Math.max(0, Math.min(maxZoom - this.context.map.getZoom(), 12)) / 2)
